...
num_cols, num_rows = max([c[0] for l in lines for c in l]), max([c[1] for l in lines for c in l]) + 1
num_cols = num_cols + 1000
floor = num_rows+1
print(floor)
...
def move_sand(grid, sx, sy):
    ...
    while get_item(grid, sx+1,sy) == '.':
        sx += 1
    if get_item(grid, sx+1, sy-1) == '.':
        return move_sand(grid, sx, sy-1)
    elif get_item(grid, sx+1, sy+1) == '.':
        return move_sand(grid, sx, sy+1)
    else:
        return sx, sy
